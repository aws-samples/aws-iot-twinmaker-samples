...
def extract_entity(asset, model, parent_id):
    entity = {  'entity_id': underscored(asset.get("name")),
                'entity_name': underscored(asset.get("name")),
                'description': asset.get("description"),
                'asset_id' : asset.get("id"),
                'component_id' : 'com.sitewise.user.' + underscored(
                                        model.get('name')) }
...
def siblings(model, asset, collect, parent_id):
    collect.append(extract_entity(asset, model, parent_id))
    ...
    for hierarchy in hierarchies:
        ...
        for child in assets:
            collect = siblings(model, child, collect, parent_id)
    return collect
    ...
    for model in models:
        assets = all_results(sw.list_assets,
                {"assetModelId":model.get('id'), "filter":"TOP_LEVEL"},
                            "assetSummaries")
        for asset in assets:
            collect = siblings(model, asset, collect, '$ROOT')
