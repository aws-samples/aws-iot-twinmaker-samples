...
WS_ROLE_ARN=$(aws iottwinmaker get-workspace --workspace-id ${WORKSPACE_ID} --region ${AWS_DEFAULT_REGION} | jq -r ".role")
...
WORKSPACE_ROLE_POLICIES=$(aws iam list-attached-role-policies --role-name $ROLE_NAME) && \
